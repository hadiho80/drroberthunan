@php
    ...
    $mediaManager = app(\App\Support\CmsMediaManager::class);
    $isVideo = $mediaManager->isVideoPath($value ?? null);
@endphp
...
<label class="{{ $class ?? '' }}">
    ...
    @endif
    <input type="file" name="{{ $name }}" accept="{{ $accept ?? 'image/*' }}">
    @if($previewUrl)
        <div class="mt-3 overflow-hidden rounded-[10px] border border-slate-200 bg-white p-3">
            @if($isVideo)
                <video controls preload="metadata" class="block max-h-[220px] w-auto max-w-full rounded-[6px] object-contain">
